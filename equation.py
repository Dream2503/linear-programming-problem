...
# CREATING A COPY OF THE VARIABLE
    ...
    def __init__(
            self, expression: Any = "0", *, coefficient: MathNum = MathNum(),
    ...
    def __add__(self, other: "Variable") -> "Variable":
        ...
        if isinstance(other, Variable) and self.variables == other.variables:
            return Variable(
                    coefficient=self.coefficient + other.coefficient, variables_dict=self.variables,
                    **self._parameters,
            )
    ...
    def __mul__(self, other: "Variable | Any") -> "Variable":
        ...
        if isinstance(other, Variable):
            ...
            try:
                return Variable(
                        coefficient=self.coefficient * MathNum(other, datatype=self._parameters["datatype"]),
    ...
    __abs__ = lambda self: Variable(
            coefficient=abs(self.coefficient), variables_dict=self.variables.copy(),
            **self._parameters,
    )
                    ...
                    for key, val in self.variables.items():
                        if key == variable:
                            ...
                            variables[first] = variables[first] + second * val if variables.get(
                                    first,
                            ) is not None else second * val

                        else:
                            variables[key] = variables[key] + val if variables.get(key) is not None else val

                    return Variable(
                            coefficient=self.coefficient * value.coefficient, variables_dict=variables,
                            **self._parameters,
                    )

            else:
                return Variable(
                        coefficient=self.coefficient * (MathNum(value) ** self.variables[variable]),
...
# CONVERTING A QUADRATIC POLYNOMIAL TO ITS RESPECTIVE PRINCIPAL AXES FORM
    ...
    __rmul__ = lambda self, other: self.__mul__(other)
    __rtruediv__ = lambda self, other: Polynomial(
            numerator_variables=self.denominator,
            denominator_variables=self.numerator, **self._parameters,
    ) * other
    __neg__ = lambda self: self * -1
    __pos__ = lambda self: self
    ...
    def _post_simplify(self) -> None:
        ...
        for element in self.numerator[1:]:
            if factor is not None and factor[0] in element.variables:
                factor[1] = min(
                        MathNum(factor[1], datatype=self._parameters["datatype"]),
    ...
    def _pre_simplify(self) -> None:
        ...
        for idx, variable in enumerate(self.numerator):
            ...
            for var, exp in variable.variables.items():
                if exp == 0:
                    ...
                    factor = Variable(
                            coefficient=MathNum(1, datatype=self._parameters["datatype"]),
        ...
        if factor is not None and main_var is not None:
            ...
            self.numerator.append(
                    Variable(
                            coefficient=main_var.coefficient,
    ...
    def roots(self) -> tuple[MathNum, ...]:
        ...
        def cubic_roots(b: MathNum, c: MathNum, d: MathNum) -> tuple[MathNum, MathNum, MathNum]:
            ...
            if check > 0:
                ...
                res: tuple[MathNum, MathNum, MathNum] = (
                    t1 - b / 3, MathNum((x2, y2.real), datatype=complex),
    ...
    copy = lambda self: Polynomial(
            numerator_variables=self.numerator, denominator_variables=self.denominator,
            **self._parameters,
    )
    substitute = lambda self, variable, value: (
        Polynomial(
                numerator_variables=[var.substitute(variable, value) for var in self.numerator],
